...
const OWNER_EMAIL = "nairobicrumbery@gmail.com";
...
mailer.verify((error) => {
  if (error) {
    console.error("Mailer failed to authenticate:", error.message);
  } else {
    console.log("Mailer ready - connected as", process.env.GMAIL_USER);
  }
});
...
async function getCategoryRules(connection) {
  ...
  return rules.reduce(
    (map, rule) => ({
      ...map,
      [rule.category]: rule.minimum_quantity,
    }),
    {}
  );
}
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (databaseProducts.length !== productIds.length) {
      return res.status(400).json({
        error:
          "One or more products are unavailable. Refresh the page and try again.",
      });
    }
    ...
    if (minimumError) {
      return res.status(400).json({ error: minimumError });
    }
    ...
    const combinedNotes = [
      notes?.trim(),
      fulfillment.landmark?.trim()
        ? `Landmark: ${fulfillment.landmark.trim()}`
        : null,
      fulfillment.preferredDate
        ? `Preferred date: ${fulfillment.preferredDate}`
        : null,
      fulfillment.preferredTime
        ? `Preferred time: ${fulfillment.preferredTime}`
        : null,
      !feeWasEstimated
        ? "Delivery fee auto-estimate failed - confirm manually."
        : null,
    ]
      .filter(Boolean)
      .join(" | ") || null;
    ...
    const [orderResult] = await connection.query(
      ...
      [
        ...
        fulfillment.type,
        deliveryAddress,
        fulfillment.landmark?.trim() || null,
        deliveryLat,
        deliveryLng,
        distanceKm,
        deliveryFee,
        deliveryEtaLabel,
        fulfillment.preferredDate || null,
        fulfillment.preferredTime || null,
        subtotal,
        total,
        "pending",
        "unpaid",
    ...
    try {
        ...
        html: `
          <div style="font-family:Arial,sans-serif;color:#35170b;max-width:650px">
            ...
            <p>
              ...
              ${fulfillment.preferredDate || "Not specified"}
              ${fulfillment.preferredTime || ""}
            </p>
            ...
            <h2 style="color:#8b0505">
              Total: KSh ${total.toLocaleString()}
            </h2>

            <p>
              Customer will submit their M-Pesa code and screenshot through the
              website. You will get a follow-up email once they do.
            </p>
          </div>
        `,
      });
      ...
      console.error(
        "Order was saved, but owner email failed:",
        emailError.message
      );
    }
    ...
    if (customerEmail) {
      try {
          ...
          html: `
            <div style="font-family:Arial,sans-serif;color:#35170b;max-width:650px;margin:auto">
              ...
              <div style="padding:28px;border:1px solid #eadbc7;border-top:0;border-radius:0 0 16px 16px;background:#fffdf8">
                ...
                <p>
                  Thank you for ordering with Nairobi Crumbery. Keep your
                  reference number handy whenever you contact us.
                </p>
                ...
                <p>
                  ...
                  ${fulfillment.type === "delivery" ? "Delivery" : "Collection"}
                  <br>
                ...
                <p>
                  ...
                  <strong style="font-size:18px;color:#8b0505">
                    Total: KSh ${total.toLocaleString()}
                  </strong>
                </p>

                <p>
                  Please complete payment by M-Pesa to
                  <strong>${SHOP_MPESA_PHONE}</strong>, then submit your M-Pesa
                  code and payment screenshot on the website.
                </p>
                  ...
                  trackingUrl
                    ? `
                      <p>
                        <a
                          href="${trackingUrl}"
                          style="display:inline-block;background:#8b0505;color:#fffaf1;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:bold"
                        >
                          Track your order
                        </a>
                      </p>
                ...
                <p style="color:#796151;font-size:13px;margin-top:24px">
                  Questions? Reply to this email and include your order
                  reference: ${orderCode}.
                </p>
              </div>
            </div>
        ...
        console.error(
          "Order was saved, but customer confirmation email failed:",
          emailError.message
        );
      }
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!phone?.trim() || !mpesaCode?.trim()) {
      return res.status(400).json({
        error: "Phone number and M-Pesa code are required.",
      });
    }
    ...
    try {
      ...
      try {
        await mailer.sendMail({
          ...
          to: OWNER_EMAIL,
          subject:
            `Payment submitted for ${orderCode} - ` +
            `KSh ${Number(rows[0].total).toLocaleString()}`,
          html: `
            <div style="font-family:Arial,sans-serif;color:#35170b;max-width:650px">
              ...
              <p>
                Check your M-Pesa messages for this exact code. If it matches
                and the amount is correct, click below to approve.
              </p>

              <p>
                <a
                  href="${approveLink}"
                  style="background:#8b0505;color:#fff;padding:12px 20px;border-radius:6px;text-decoration:none;display:inline-block"
                >
                  Approve Payment
                </a>
              </p>
              ...
              <p style="color:#888;font-size:13px">
                If the code doesn't match, do not approve it. Contact the
                customer instead.
              </p>
            </div>
          `,
        ...
        console.error(
          "Payment submitted, but alert email failed:",
          emailError.message
        );
      }
      ...
      return res.status(500).json({
        error: "Could not submit payment details. Please try again.",
      });
    }
  }
);
...
router.get("/:orderCode/approve-payment", async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res.send(
        `<p style="font-family:sans-serif">
          Order ${orderCode} was already approved, or no submitted payment was found.
        </p>`
      );
    }
    ...
    return res.send(
      `<p style="font-family:sans-serif">
        Payment approved for order ${orderCode}. Order marked as confirmed.
      </p>`
    );
  } catch (error) {
...
router.get("/:orderCode/track", async (req, res) => {
  ...
  if (!phone?.trim()) {
    return res.status(400).json({
      error: "Please provide the phone number used on the order.",
    });
  }
